...
public class ujOktatoController {
    ...
    private void uresMezo() {
        boolean disableButtons = oNev.getText().trim().isEmpty() || oTanszek.getText().trim().isEmpty() || oEmail.getText().trim().isEmpty();
        hozzaadGomb.setDisable(disableButtons);
    }

    @FXML
    private void hozzaad() throws IOException{
        Oktato ujOktato = new Oktato(oNev.getText(), oTanszek.getText(), new ArrayList<>(), oEmail.getText(),
                "", new ArrayList<>());
        if (ujOktato.oktatoFeltolto()) {
            ...
            dialogPane.getStylesheets().add(
                    getClass().getResource("dialogCSS.css").toExternalForm());
            dialogPane.getStyleClass().add("dialogCSS");
            alert.showAndWait();
            visszaOktatooldalra();
        } else {
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("Hib's emai;");
